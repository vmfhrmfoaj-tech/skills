...
개발자가 함수 시그니처, 인터페이스 형태, 소유권, 데이터 흐름, 실패 의미 또는 테스트 경계를 새로 발명하지 않고 이 설계 문서만으로 바로 구현할 수 있을 만큼 충분히 상세하게 설계한다.

언어에 적합한 시그니처 또는 의사 코드를 선택적 스케치가 아니라 필수 계약으로 제시한다. 최종 사용자 또는 개발자 같은 공통 기능 사용자와 연결된 인터페이스, 그리고 설계에서 이름 붙인 모든 중요한 함수 또는 인터페이스에는 항상 시그니처를 포함한다. 함수, 메서드, 인터페이스, 이벤트, 콜백 또는 명령 이름, 입력 매개변수와 타입, 반환 타입 또는 결과 형태, 비동기·동시성 또는 해당 패러다임의 실행 의미, 호출자에게 보이는 오류/결과 계약, 중요한 부수 효과 또는 멱등성 제약을 포함한다. 관련이 있는 경우 호출자에게 보이는 불변조건, 유효성 검사, 결과, 오류, 순서, 부수 효과 및 성능 제약 조건을 설명한다. 구현 세부사항은 인터페이스 뒤에 숨긴다.
...
다이어그램이 구현 모호성을 줄인다면 의존성/구조, 시퀀스, 상태, 데이터 흐름, 수명주기 또는 모듈 경계 다이어그램처럼 유용한 만큼 포함한다. 다이어그램은 장식이 아니라 구현 결정을 명확히 해야 하며, 모든 다이어그램은 작성된 책임, 호출 방향, 상태 전이 및 데이터 흐름과 일치해야 한다.
...
- 설계 문서가 개발자가 함수 시그니처 또는 인터페이스 형태를 새로 발명하지 않고 구현할 수 있을 만큼 상세하다.
- 최종 사용자, 개발자 같은 공통 기능 사용자, 그리고 설계에서 이름 붙인 모든 중요한 함수/인터페이스와 연결된 인터페이스에 시그니처가 포함되어 있다.
- 파일 배치가 책임 소유권을 따른다.
- 필요한 다이어그램이 포함되어 있으며, 모든 다이어그램이 작성된 의존성 및 동작과 일치한다.
